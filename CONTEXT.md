...
**Agent**:
A coding tool installed on the machine that can consume capabilities from the **Library** (e.g. Cursor, Claude Code, Pi, OpenCode, Devin, Codex).
...
- Cursor and Claude Code are expected **Install targets**; Pi, OpenCode and Devin are also supported when detected; other **Agents** are included only when detected
...
- Devin global skills path: `~/.config/devin/skills` (or `$XDG_CONFIG_HOME/devin/skills`)
